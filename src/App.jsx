...
export default function InventoryApp() {
  ...
  async function saveAdd() {
    ...
    setSaving(true);
    try {
      await sbFetch("inventory", { method: "POST", body: JSON.stringify({ name: form.name, sku: form.sku, category: form.category, stock: Number(form.stock) || 0, unit: form.unit }) });
  ...
  async function handleStockAction() {
    ...
    try {
      ...
      await sbFetch("inventory_log", { method: "POST", body: JSON.stringify({ inventory_id: item.id, item_name: item.name, action: stockAction.type === "in" ? "入庫" : "出庫", quantity: qty, user_name: userName }) });
  ...
  async function addCategory() {
    ...
    try {
      await sbFetch("categories", { method: "POST", body: JSON.stringify({ name: newCategory.trim() }) });
      setNewCategory("");
      await loadAll();
